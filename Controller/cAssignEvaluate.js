...
var fnDropdownListAsignDep=function(department_id,paramSelectAll){
	...
	$.ajax({
		...
		success:function(data){
			...
			var htmlDropDrowList="";
			htmlDropDrowList+="<select style='width:100%;' id=\"assign_department_id\" name=\"assign_department_id\" class=\"\" >";
			//htmlDropDrowList+="<option value=\"All\" >ทั้งหมด</option>";
				$.each(data,function(index,indexEntry){
	...
	var fnDropdownListAppralisalAssignKpi=function(year,appraisal_period_id,paramSelectAll="selectAll"){
		...
		$.ajax({
			...
			success:function(data){
				...
				var htmlDropDrowList="";
				htmlDropDrowList+="<select  style='width:100%;'id=\"appraisal_period_assign_kpi\" name=\"appraisal_period_assign_kpi\" class=\"\" style=\"width:auto;\">";
				//htmlDropDrowList+="<option value=\"all\">ทั้งหมด</option>";
					$.each(data,function(index,indexEntry){
...
var fnDropdownListYear=function(kpi_year){
	...
	$.ajax({
		...
		success:function(data){
			...
			var htmlDropDrowList="";
			htmlDropDrowList+="<select id=\"assign_year\" name=\"assign_year\" class=\"\" style=\"width:100%;\">";
				$.each(data,function(index,indexEntry){
		...
		success:function(data){
			...
			$.each($(".confirm_flag_complete"),function(index,indexEntry){
				...
				if($("#check_status-"+appraisalYear+"-"+appraisalPeriodID+"-"+departmentID+"-"+positionID+"-"+empID).hasClass('confirm_flag_complete')){
					...
					$("#status_confirm_flag-"+appraisalYear+"-"+appraisalPeriodID+"-"+departmentID+"-"+positionID+"-"+empID).html("ส่งประเมินเรียบร้อย").parent().parent().removeClass("alert-danger").addClass("alert-success");
...
$(document).ready(function(){
	...
	searchAssignMasterKPIFn();
		...
		if(sessionStorage.getItem('checkMobile')=='mobile'){
			...
			$(".imagePerson").css({"width":"100%"});
			$(".sendKpiAssignByEmp").css({"width":"100%"});
			$(".assignKpiByEmp").css({"width":"100%"});
			...
			$(".imagePerson").css({"width":"auto"});
			$(".sendKpiAssignByEmp").css({"width":"auto"});
			$(".assignKpiByEmp").css({"width":"auto"});
		}
